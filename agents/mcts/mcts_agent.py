...
def serialize_state(state):
    """Serializes a HanabiState object into a dictionary."""
    serialized_state = {
        ...
        'discard_pile': [
            {'color': card.color(), 'rank': card.rank()}
            for card in state.discard_pile()
        ],
        'hands': [
            [{'color': card.color(), 'rank': card.rank()} for card in hand]
        ...
        'move_history': [
            {
                ...
                'card_index': move.move().card_index(),
...
def deserialize_state(state_dict, game):
    """Deserializes a dictionary into a HanabiState object."""
    # Initialize a new state
    new_state = pyhanabi.HanabiState(game)

    # Restore basic game state (e.g., fireworks, life tokens, etc.)
    new_state._cur_player = state_dict['cur_player']
    new_state._life_tokens = state_dict['life_tokens']
    new_state._information_tokens = state_dict['information_tokens']
    new_state._turns_to_play = state_dict['turns_to_play']
    new_state._deck_size = state_dict['deck_size']  # Added missing deck size restoration

    # Restore fireworks
    new_state._fireworks = state_dict['fireworks']

    # Restore discard pile
    for card in state_dict['discard_pile']:
        discard_card = pyhanabi.HanabiCard(card['color'], card['rank'])
        new_state.discard_pile().append(discard_card)

    # Restore hands
    for pid, hand in enumerate(state_dict['hands']):
        for card in hand:
            hand_card = pyhanabi.HanabiCard(card['color'], card['rank'])
            new_state.player_hands()[pid].append(hand_card)

    # Restore move history
    for move_data in state_dict['move_history']:
        move_type = pyhanabi.HanabiMoveType[move_data['type']]
        move = None  # Initialize the move variable to avoid UnboundLocalError
        if move_type == pyhanabi.HanabiMoveType.PLAY:
            move = pyhanabi.HanabiMove.get_play_move(move_data['card_index'])
        elif move_type == pyhanabi.HanabiMoveType.DISCARD:
            move = pyhanabi.HanabiMove.get_discard_move(move_data['card_index'])
        elif move_type == pyhanabi.HanabiMoveType.REVEAL_COLOR:
            move = pyhanabi.HanabiMove.get_reveal_color_move(move_data['card_index'], move_data['color'])
        elif move_type == pyhanabi.HanabiMoveType.REVEAL_RANK:
            move = pyhanabi.HanabiMove.get_reveal_rank_move(move_data['card_index'], move_data['rank'])
        else:
            # If the move type is unknown, handle it appropriately (e.g., log an error, skip the move, etc.)
            print(f"Unknown move type: {move_data['type']}. Skipping this move.")
